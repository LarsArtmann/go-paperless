...
| Bump consumer flakes to Go 1.27 before `go get`-ing v0.1.1+     | 🟢 `DONE` | High   | ~1h    | `go.mod` floors `go 1.27` (normalized from the `1.27.1` patch floor); bank-sync/InboxClean run `go_1_26` in their flakes and fail to build against this module until bumped (`docs/status/…15-39….md` f-30) |
| Decide: retire bank-sync's trimmed fork in favor of this module | 🔵 `BLOCKED` | High   | ~2h    | `AGENTS.md` scope note calls this module a drop-in replacement for the fork; decision + migration belongs to bank-sync (`docs/status/…15-39….md` f-31)                                                      |

## Medium Impact

| Task                                                                             | Status    | Impact | Effort | Evidence                                                                                                                                                                                                    |
| -------------------------------------------------------------------------------- | --------- | ------ | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T18: integration test scaffold (`//go:build integration`)                        | 🟢 `DONE` | Med    | ~3h    | `integration_test.go`: env-driven (`PAPERLESS_INTEGRATION_URL`/`PAPERLESS_INTEGRATION_TOKEN`), fails fast when unset (the build tag already keeps it out of normal runs); covers ping + all five listings; upload→poll→reconcile e2e NOT yet in the scaffold (`docs/status/…15-39….md` f-18) |
| CI: add a `test-race` job + verify the `pull_request` trigger                    | 🟢 `DONE` | Med    | ~30min | `ci.yml` runs `nix flake check`/govulncheck/gosec on push+PR but no race job; only the push path was exercised (`docs/status/…15-39….md` f-22, f-23)                                                        |
...
| T22: ADR 0001 — API-version policy beyond v10                                    | 🟢 `DONE` | Med    | ~1h    | Probe-vs-negotiate-vs-break decision record the ROADMAP's confidence theme references (`docs/status/…15-39….md` f-20)                                                                                       |
...
| Hook-coverage for `ProbeCapabilities` requests                 | 🟢 `DONE` | Low    | ~15min | Probes go through `doRequestDetail`, so hooks fire there too — assert it (`docs/status/…15-39….md` f-34)                                                      |
