...
5. **Ghost config: `dprint.json`.** Nothing in the repo invokes dprint — formatting runs through treefmt (gofumpt/goimports/golines/nixfmt) via flake + `nix fmt`. The dprint config is dead weight that *looks* authoritative. Wire it or delete it.
...
1. **Gate-first audits.** Run the full canonical gate *before* touching anything, so pre-existing vs. introduced failures are separable. This session ran apps early but `nix flake check` only at the end — the fmt-app bug and sandbox failure surfaced late.
...
2. **Publishing intent:** should this repo be pushed with a GitHub Release + pkg.go.dev propagation *now*, and should CI go live in the same step? I did not touch remotes this session and won't without instruction.
...
*Point-in-time snapshot — goes stale by design. Annotate non-destructively via docs-health ANNOTATE mode; do not rewrite.*
