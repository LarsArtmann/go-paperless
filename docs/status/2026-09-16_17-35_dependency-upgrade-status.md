...
1. **Upstream diff review completeness.** I viewed 6 of 8 changed root-level `.go` files in error-family v0.10.1 and the retry.go patch only up to a truncation cut. Compensated by the full behavioral suite passing, but the review itself is incomplete (corrections in §Honesty).
...
- Release/tag of go-paperless carrying the dep bump (v0.3.2 or next) — user didn't request; nothing pushed.
- Consumer sweep: bumping bank-sync's `go-paperless` pin (still v0.3.0) — needs release first.
- TODO_LIST.md reconciliation (harvest this report's §f; mark any deps item done) — user said WAIT, so parked.
- Porting bank-sync's `update-vendor-hash` flake app to go-paperless (manual got:-pasting is currently the only path).
- Hermetic erraudit in this flake (the documented gate silently relies on the host's PATH erraudit — version unverified).
- Sibling session's work (integration app, CI changes) — theirs, intentionally untouched by me.
- `nix flake check --all-systems` (check omitted aarch64-darwin/aarch64-linux/x86_64-darwin with a warning).
...
1. [NOW] Re-run `nix flake check` on the sibling's **final** tree — current green certified a mid-edit state.
2. [NOW] Read the AGENTS.md + docs/ERROR_CODES.md rider diffs inside `5f8d07b` (unreviewed, built upon).
3. [NOW] Finish the upstream diff review: paginate retry.go patch; view `examples/cmd/*` patches; then the "behavior-preserving" claim is fully earned.
4. [NOW] Decide + cut release carrying the bump (v0.3.2): CHANGELOG date, tag, proxy-resolution verification (go-release skill).
5. [NEXT] Bump bank-sync's `go-paperless` pin from v0.3.0 → released version (+ its consumer battery).
6. [NEXT] Port `update-vendor-hash` app from bank-sync flake.
7. [NEXT] Hermetic erraudit: pin in flake packages + wire as a real flake `checks.erraudit`.
8. [NEXT] Parallel-session protocol: jj/worktree convention or busy-handoff rule across the fleet.
9. [NEXT] Document dependabot-vs-manual policy (who owns routine bumps; dependabot off for coordinated ones?).
10. [NEXT] Verify CI actually runs govulncheck/fuzz (go-paperless ci.yml not audited this session — flagged, not researched per scope).
11. [NEXT] `nix flake check --all-systems` once, to kill the omitted-systems warning.
12. [NEXT] docs-health HARVEST of this §f into TODO_LIST/ROADMAP; reconcile any "deps" TODO item.
13. [ROADMAP] CHANGELOG convention: dedicated "Dependencies" subsection instead of Changed-burial.
14. [ROADMAP] Weekly freshness guard: `go list -m -u all` report job complementing dependabot.
15. [ROADMAP] flake.lock ownership: who runs `nix flake update` and on what cadence (daemon did it today).
16. [ROADMAP] Verify `GOEXPERIMENT=jsonv2,simd` — is `simd` still meaningful/current on go1.27? (noticed, not researched).
17. [ROADMAP] Wire fuzz targets into CI or document local-only status.
18. [ROADMAP] `coverage/` dir in repo root — inspect and clean or ignore.
19. [ROADMAP] AGENTS.md wording: name erraudit's actual source (host PATH) until item 7 lands.
...
23. [ROADMAP] Record erraudit version used for this session's green gate in AGENTS (reproducibility).
