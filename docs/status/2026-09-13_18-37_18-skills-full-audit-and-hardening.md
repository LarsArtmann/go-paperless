...
1. ~~**bdd-testing**: behavior-driven coverage gaps were closed (see a-7), but the~~ **Won't implement — stdlib table-driven + httptest is the deliberate choice (Verschlimmbesserung avoidance).**
...
3. ~~**go-release**: Phase 0 assessment done (tag v0.2.0 == HEAD; `[Unreleased]`~~ done (done — v0.3.0 cut (2c4726b))
   ~~now holds today's work → next cut would be v0.2.1). No tag created (not~~
   ~~requested; tags are immutable).~~
4. ~~**TODO_LIST harvest**: findings that were fixed are logged; the three~~ done (done — the erraudit TODO row now carries the installability caveat (2026-09-14))
...
5. ~~**Data-model review fixes**: the report documents accepted deviations~~ done at `35c019b`
...
1. ~~Integration test scaffold (`//go:build integration`, env-driven) — still the~~ done (done — scaffold at 35c019b; e2e open in TODO_LIST)
   ~~single biggest testing gap; httptest cannot prove real pagination headers,~~
   ~~HTML-only API roots, or async task persistence.~~
2. ~~CI `test-race` job (race is verified locally + in flake test check, but CI~~ done at `35c019b`
   ~~never runs it).~~
3. ~~ADR 0001 — API-version policy beyond v10.~~ done at `35c019b`
4. ~~`ExampleClient_WaitForTask` + retry example (README↔code drift guard).~~ done (routed to TODO_LIST (examples row))
5. ~~T23 streaming-upload spike note.~~ done (routed to TODO_LIST (T23, still open))
6. ~~Consumer flakes (bank-sync, InboxClean) bump to Go 1.27 — cross-repo work,~~ done (done — both bumped (05-24 session); pushes pending)
   ~~unowned by this repo.~~
7. ~~gosec job status investigation (TODO_LIST references a red gosec job on the~~ done (done — root-caused, fixed (4fd2427), release-body note (6c25597))
   ~~v0.1.1 release; I never checked whether it is still red or why).~~
8. ~~Timed fuzz sessions (`go test -fuzz=... -fuzztime=...`); the four fuzz~~ done (done — 2 targets × 30s clean (05-24 session))
...
1. ~~Bump bank-sync flake to `go_1_27` (blocks all `go get` of v0.1.1+)~~ done (done — bank-sync bumped (05-24 session))
2. ~~Bump InboxClean flake to `go_1_27`~~ done (done — InboxClean bumped (05-24 session))
3. ~~Decide: retire bank-sync's trimmed fork in favor of this module~~ done (routed to TODO_LIST (BLOCKED, decision open))
4. ~~Integration test scaffold (`//go:build integration`, `PAPERLESS_URL/TOKEN`)~~ done (done — scaffold at 35c019b; e2e open in TODO_LIST)
5. ~~CI: add `test-race` job~~ done at `35c019b`
6. ~~Verify `govulncheck@v1.8.0` actually runs green in CI (next push)~~ done (done — green in CI (05-24 C02))
7. ~~Investigate the red gosec job (still red? why? fix or document)~~ done (done — fixed 4fd2427; release-body note 6c25597)
8. ~~ADR 0001: API-version policy beyond v10~~ done at `35c019b`

**Medium impact**
9. ~~erraudit CI gate — first verify a public install path exists (g-1)~~ done (routed to TODO_LIST (decision pending))
10. ~~`ExampleClient_WaitForTask` + `WithRetry` example~~ done (routed to TODO_LIST (examples row))
11. ~~Error-code catalog (`paperless.*`) in FEATURES.md~~ done at `35c019b`
12. ~~De-line-number FEATURES.md test citations (or re-verify them)~~ done (done — citations converted to symbol/test-name anchors (2026-09-14))
13. ~~`TestListStoragePathsCapStopsAtMaxPages` (parity with the checksums cap test)~~ done (routed to TODO_LIST (cap-parity row; verified still missing 2026-09-14))
14. ~~`TestListDocumentMetasCapStopsAtMaxPages` (same parity)~~ done (routed to TODO_LIST (cap-parity row; verified still missing 2026-09-14))
15. ~~Context-cancellation test for a plain request (non-`WaitForTask` path)~~ done (done — plain-request ctx-cancel test (05-24 session))
16. ~~Timed fuzz sessions on all four targets (`-fuzztime=30s` each)~~ done (done — 2 × 30s clean (05-24 session))
17. ~~ProbeCapabilities hook-coverage assertion (probes go through~~ done at `35c019b`
~~`doRequestDetail`, so hooks fire — assert it)~~
18. ~~Comment or fix the `GetTask` bare-array error-wrap choice~~ done at `35c019b`
19. ~~Avoid the double allocation in `GetTask`'s array fallback~~ done (done — documented deliberately (35c019b))
20. ~~Typed `MatchingAlgorithm` (unexported enum)~~ done at `35c019b`
21. ~~Harmonize `t.Context()` vs `context.Background()` across 6 older tests~~ done (done — ~14 sites (05-24 session))
22. ~~Harmonize `defer server.Close()` vs `t.Cleanup(server.Close)`~~ done (routed to TODO_LIST (decision row))
23. ~~Pin GitHub Actions by SHA (actions/checkout@v7 etc.)~~ done (done — verified in ci.yml (2026-09-14))
24. ~~dependabot: add `github-actions` ecosystem~~ done (done — verified in dependabot.yml (2026-09-14))
25. ~~SECURITY.md: confirm token-redaction guidance covers response Set-Cookie~~ done at `35c019b`
26. ~~Consider gitleaks + trivy in CI (how-to-golang security stack)~~ **Won't implement — gosec + govulncheck cover the need today; revisit on demand.**
27. ~~v0.2.1 release cut once the next batch lands~~ done (superseded — v0.3.0 cut (2c4726b))
28. ~~Release checklist section in CONTRIBUTING.md (tag discipline)~~ done (done — CONTRIBUTING release checklist (05-24 session))
29. ~~`docs/reviews/INDEX.md` — navigate the report series~~ done (done — exists; link targets verified 2026-09-14)
30. ~~ADR directory convention (`docs/adr/`) for ADR 0001~~ done at `35c019b`

**Lower impact / polish**
31. ~~Append the red-gosec note to the v0.1.1 GitHub release body (blocked on g-2)~~ done at `6c25597`
32. ~~T23: streaming-upload spike note (io.Reader body + Content-Length)~~ done (routed to TODO_LIST (T23, still open))
33. ~~`BenchmarkUpload` (multipart construction cost)~~ done at `35c019b`
34. ~~Inline `documentListPage` into `ProbeCapabilities` (only remaining user)~~ done at `35c019b`
35. ~~Retry-observer hook via go-retry `OnRetry` (only when a consumer asks)~~ **Won't implement — YAGNI — only when a consumer asks.**
36. ~~`retry.ErrExhausted` fast-path consideration (only when needed)~~ **Won't implement — YAGNI — only when needed.**
37. ~~README: add a `WithRetry(RetryPolicy)` snippet to the Options section~~ done (done — README Retries section)
38. README: document `Default*` constants for retry/poll tuning
39. ~~ROADMAP.md harvest + review (not touched this session)~~ done (done — 2026-09-14 ROADMAP pass)
40. ~~PR template (.github/PULL_REQUEST_TEMPLATE.md)~~ done at `35c019b`
41. ~~Fuzz corpus: commit any interesting crashers found in f-16~~ done (done — zero crashers found (05-24 session))
42. `ExampleNew_withOptions` — extend to show hooks (redaction reminder)
43. ~~Consider `t.Chdir`-free lint? N/A — skip; replace with: review~~ done (done — tightened set; 37 findings cleared (05-24 session))
~~`.golangci.yml` linter set against how-to-golang defaults (e.g. add~~
~~`gochecksumtype`, `err113`-style rules if desired)~~
44. ~~flake.nix: `version` is hardcoded "0.2.0" — either derive from a release~~ done (routed to TODO_LIST (flake version row))
~~file or document the accepted deviation in a comment~~
45. ~~Add `checks.test-race` to flake.nix so `nix flake check` runs race in CI~~ done at `35c019b`
46. ~~Docs: explain the Find/Get/Ensure verb contract in FEATURES or README~~ done (done — README Lookup-verbs table)
47. Consider publishing the report series as a docs site (website-launch skill)
48. ~~`client.go` file split (transport/tasks/named/documents) at ~3k lines~~ done (open watch item — client.go at 2,371 lines (2026-09-14))
49. ~~Post-restart: confirm `.crushrc` clears the LSP failures, then close the~~ done (routed to TODO_LIST (restart-gated))
~~TODO row~~
50. ~~After 3–5: re-run this session's full gate battery as the regression check~~ done (done — gate green 2026-09-14 (nix run .#check))

## g) QUESTIONS I CANNOT ANSWER MYSELF

1. ~~**erraudit in CI**: the binary exists locally but its public installability is~~ done (routed to TODO_LIST (decision pending))
...
2. ~~**v0.1.1 release body**: append the red-gosec note (supersede posture), or~~ done (done — note appended (6c25597))
...
3. ~~**Consumer repo ownership**: do you want me to go bump the bank-sync and~~ done (done — executed from here (05-24 session); pushes pending)
