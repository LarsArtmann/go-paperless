...
| --- | ------------------------------------------------------------------------------------------------------------------------------------ | ------- | --------------------------- | ------ | ----------------------------------------------------------------------------------------- | ----------------------- |
| ~~C01~~ | ~~Land in-flight feature: commit notes/share-links/saved-views impl + CHANGELOG entry, full gate battery, push~~ done at `77a8e00` | ~~1%→51%~~ | ~~Critical~~ | ~~60m~~ | ~~Ships 3 API families; un-orphans committed tests; first real CI run of pinned govulncheck~~ | ~~—~~ |
| ~~C02~~ | ~~CI triage of the C01 push: govulncheck@v1.8.0 green? gosec still red? PR trigger works? Fix or document findings~~ done — CI verified per-job green incl. the PR path (run 34800579700, 05-24 session) | ~~1%→51%~~ | ~~Critical~~ | ~~50m~~ | ~~Turns CI from "assumed" to "verified"; closes two open TODO evidence gaps~~ | ~~C01~~ |
| ~~C03~~ | ~~Migrate `ListShareLinks` + `ListSavedViews` to `fetchAllPages[T]` + cap-parity tests for both~~ done at `35c019b` | ~~4%→64%~~ | ~~High~~ | ~~45m~~ | ~~Pagination policy back to one home; new families get the same 100-page-cap guarantee~~ | ~~C01~~ |
| ~~C04~~ | ~~Docs batch: FEATURES rows ×3 families, README feature bullets, de-stale FEATURES line citations → test names~~ done — FEATURES/README rows for the three families shipped with v0.3.0 (2c4726b) | ~~4%→64%~~ | ~~High~~ | ~~60m~~ | ~~Docs match the code; consumers discover the new APIs~~ | ~~C01~~ |
| ~~C05~~ | ~~Examples batch: `ExampleClient_WaitForTask`, `WithRetry` policy example, one new-family example (notes)~~ **Won't implement — consciously dropped; the examples gap lives in TODO_LIST.** | ~~4%→64%~~ | ~~Med~~ | ~~45m~~ | ~~README↔code drift guard; onboarding for the new surface~~ | ~~C01~~ |
| ~~C06~~ | ~~Consumer flakes → `go_1_27`: bank-sync + InboxClean (cross-repo, `go get` this module, build+test)~~ done — both consumers bumped to go 1.27 + v0.3.0, gates green locally; pushes still pending user go | ~~4%→64%~~ | ~~Critical (delivery blocker)~~ | ~~60m~~ | ~~Consumers can finally consume v0.1.1+~~ | ~~— (parallel-safe)~~ |
| ~~C07~~ | ~~CI `test-race` job + `flake.nix` `checks.test-race`~~ done at `35c019b` | ~~20%→80%~~ | ~~Med~~ | ~~40m~~ | ~~Race safety enforced, not just locally verified~~ | ~~C02~~ |
...
| ~~C10~~ | ~~ADR 0001 — API-version policy beyond v10 + `docs/adr/` scaffold~~ done at `35c019b` | ~~20%→80%~~ | ~~Med~~ | ~~50m~~ | ~~Decision record the ROADMAP's confidence theme references~~ | ~~—~~ |
| ~~C11~~ | ~~Integration test scaffold (`//go:build integration`, env-driven, skip-if-unset)~~ done at `35c019b` | ~~20%→80%~~ | ~~High~~ | ~~100m~~ | ~~Only test class that exercises real server behavior~~ | ~~C01~~ |
| ~~C12~~ | ~~Error-code catalog `paperless.*` (FEATURES table or docs page)~~ done at `35c019b` | ~~20%→80%~~ | ~~Med~~ | ~~45m~~ | ~~Consumers can switch on codes without reading SDK source~~ | ~~C01~~ |
| ~~C13~~ | ~~Timed fuzz sessions ×4 (`-fuzztime=30s`), commit any crashers, coverage re-run on enlarged surface~~ done — 2 targets × 30s, zero crashers; 88.6% recorded (05-24 session) | ~~20%→80%~~ | ~~Med~~ | ~~40m~~ | ~~Deepens the only cheap continuous verification we have~~ | ~~C01~~ |
...
| ~~C15~~ | ~~Test polish batch: plain-request ctx-cancel test, harmonize `t.Context()` (6 sites), harmonize `defer server.Close()` vs `t.Cleanup`~~ done — t.Context() harmonized (~14 sites); the defer-vs-t.Cleanup call remains open in TODO_LIST | ~~→100%~~ | ~~Low-Med~~ | ~~60m~~ | ~~Consistency; future diffs stay clean~~ | ~~—~~ |
| ~~C16~~ | ~~`GetTask` bare-array fallback: document or fix the error-wrap choice; note the double allocation~~ done at `35c019b` | ~~→100%~~ | ~~Low~~ | ~~30m~~ | ~~Honesty in an obscure path~~ | ~~—~~ |
| ~~C17~~ | ~~Typed `MatchingAlgorithm` enum (unexported) replacing bare int constants~~ done at `35c019b` | ~~→100%~~ | ~~Low~~ | ~~30m~~ | ~~Type safety in the tag self-heal path~~ | ~~—~~ |
| ~~C18~~ | ~~`ProbeCapabilities` hook-coverage assertion test~~ done at `35c019b` | ~~→100%~~ | ~~Low~~ | ~~30m~~ | ~~Closes TODO f-34 evidence row~~ | ~~—~~ |
| ~~C19~~ | ~~Supply chain: pin GitHub Actions by SHA; dependabot `github-actions` ecosystem; gitleaks/trivy decision~~ done — SHA-pinned actions + github-actions dependabot group verified in-tree 2026-09-14; gitleaks/trivy not adopted | ~~→100%~~ | ~~Med~~ | ~~45m~~ | ~~Hardens the pipeline that C02 just verified~~ | ~~C02~~ |
| ~~C20~~ | ~~SECURITY.md: Set-Cookie redaction note (response hooks)~~ done at `35c019b` | ~~→100%~~ | ~~Low~~ | ~~30m~~ | ~~Prevents a real secret-leak footgun~~ | ~~—~~ |
| ~~C21~~ | ~~README polish: `WithRetry` snippet, `Default*` constants, Find/Get/Ensure verb contract~~ done at `35c019b` | ~~→100%~~ | ~~Low-Med~~ | ~~35m~~ | ~~Onboarding quality~~ | ~~C04~~ |
| ~~C22~~ | ~~Docs plumbing: `docs/reviews/INDEX.md`, release checklist in CONTRIBUTING, ROADMAP harvest, flake.nix version comment~~ done — docs/reviews/INDEX.md + CONTRIBUTING release checklist + ROADMAP note | ~~→100%~~ | ~~Low-Med~~ | ~~60m~~ | ~~Series navigation; repeatable releases~~ | ~~C09~~ |
| ~~C23~~ | ~~Benchmarks: `BenchmarkUpload` (multipart cost); inline `documentListPage` into probe~~ done at `35c019b` | ~~→100%~~ | ~~Low~~ | ~~40m~~ | ~~Performance visibility; dead type cleanup~~ | ~~—~~ |
| ~~C24~~ | ~~PR template + `.golangci.yml` review against how-to-golang defaults~~ done at `35c019b` | ~~→100%~~ | ~~Low~~ | ~~40m~~ | ~~Contribution quality; lint alignment~~ | ~~—~~ |
...
| ~~C26~~ | ~~v0.1.1 release-body note (red gosec) — append or leave, per your answer~~ done at `6c25597` | ~~→100%~~ | ~~Low~~ | ~~30m~~ | ~~Closes the oldest BLOCKED row~~ | ~~Q2, C02~~ |
...
> _Annotated 2026-09-14:_ per-row verdicts for the fine tasks are indexed by
> the coarse markers above — every F-task is a sub-step of its C-task, and
> each annotated C-row carries the hash/evidence. F-rows under unmarked
> C-rows (C08 erraudit, C14 fork retirement, C25 LSP restart) remain open;
> C05's fine tasks (F22–F24) were consciously dropped (see the C05 marker).
...
_(F67–F72 overflow the 66 count by design — they are the tail; execute in order, stop when value runs out.)_
