...
// reports failures through tb.Errorf, and this stub captures them so tests
...
func TestUnknownRouteAnswers404AndFailsTest(t *testing.T) {
	...
	response, err := http.DefaultClient.Do(
		newTestRequest(t, http.MethodGet, server.URL()+"/api/unknown/"),
	)
	if err != nil {
	...
	if got := stub.errorCount(); got != 1 {
		t.Errorf("tb.Errorf call count = %d, want 1 for unknown route", got)
	}
}
...
func TestRequestsAreRecorded(t *testing.T) {
	...
	request := newTestRequest(t, http.MethodGet, server.URL()+"/api/missing/?page=1&page_size=1")
